...
export const MovieContextProvider = ({ children }) => {
  ...
  const [searchYear, setSearchYear] = useState([1970, 2021]);
  ...
  useEffect(() => {
    ...
    if (currentPage.name === searchName && currentPage.type === searchType && searchName !== "") {
      if (currentPage.page < (searchResult.total / 10)) {
        axios
          .get(`${config.baseUrl}?apikey=${config.apiKey}&s=${searchName}&type=${searchType}&page=${currentPage.page + 1}`)
          .then(
            res => {
  ...
  useEffect(() => {
    if (currentMovieId !== "") {
      axios
        .get(`${config.baseUrl}?apikey=${config.apiKey}&i=${currentMovieId}`)
        .then(
          res => {
            setMovie(res.data);
